...
(async () => {
  ...
  try {
    ...
    await page.getByRole('tab',{name:'キー割り当て',exact:true}).click();
    ...
    await page.getByRole('tab',{name:'入力設定',exact:true}).click();
    ...
    await page.waitForFunction(()=>window.writes.some(x=>x.id===1&&x.data[30]===1));
    ...
    await page.screenshot({path:path.resolve('../../../outputs/settings-input-1.10.14.png'),fullPage:true});
    await page.getByRole('tab',{name:'LED設定',exact:true}).click();
    ...
    await page.screenshot({path:path.resolve('../../../outputs/settings-led-1.10.14.png'),fullPage:true});
    await page.getByRole('tab',{name:'モニター',exact:true}).click();
    ...
    await page.screenshot({path:path.resolve('../../../outputs/controller-monitor-1.10.38.png'),fullPage:true});
    await page.getByRole('tab',{name:'入力設定',exact:true}).click();
