...
export class DfuDevice extends EventEmitter {
  ...
  async downloadFirmware(firmware: MemoryMap): Promise<void> {
    ...
    if (!fixedFirmware.size) throw new Error('Empty firmware');
    ...
    for await (const [address, segment] of fixedFirmware) {
      const progress = i++ / fixedFirmware.size;
...
function convertMapToFixedLength(inputMap: MemoryMap, fixedLength: number) {
  const outputMap = new Map<number, Uint8Array>();
  for (const [address, bytes] of inputMap) {
    if (address < 0 || address + bytes.length > 0x10000) {
      throw new Error('Firmware exceeds the supported 64 KiB DFU address range');
    }
    for (let offset = 0; offset < bytes.length; offset += fixedLength) {
      outputMap.set(address + offset, bytes.slice(offset, offset + fixedLength));
    }
  }
  return new Map([...outputMap].sort(([a], [b]) => a - b));
}
