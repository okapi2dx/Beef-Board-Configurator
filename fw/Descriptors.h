...
typedef struct {
  ...
  USB_Descriptor_Endpoint_t HID_ConfigReportINEndpoint;
...
enum {
  ...
  HID_REPORTID_Diagnostics = 0x04,
  HID_REPORTID_DiagnosticsStream = 0x05
};
...
const USB_Descriptor_HIDReport_Datatype_t PROGMEM ConfigHIDReport[] {
  ...
  HID_RI_COLLECTION(8, 0x01),
    ...
    HID_RI_USAGE(8, 0x03),
    HID_RI_REPORT_COUNT(8, sizeof(uint32_t) + 3),
    HID_RI_REPORT_SIZE(8, 0x08),
    HID_RI_FEATURE(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),

    HID_RI_REPORT_ID(8, HID_REPORTID_Diagnostics),
    HID_RI_USAGE(8, 0x04),
    HID_RI_REPORT_COUNT(8, sizeof(diagnostics_report)),
    HID_RI_REPORT_SIZE(8, 0x08),
    HID_RI_FEATURE(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),

    // Stream diagnostics over an interrupt IN endpoint so the desktop
    // monitor never has to continuously issue blocking control transfers.
    HID_RI_REPORT_ID(8, HID_REPORTID_DiagnosticsStream),
    HID_RI_USAGE(8, 0x05),
    HID_RI_REPORT_COUNT(8, sizeof(diagnostics_report)),
    HID_RI_REPORT_SIZE(8, 0x08),
    HID_RI_INPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),
...
// Diagnostics stream for the desktop monitor. Endpoint 6 is otherwise free.
#define CONFIG_IN_EPADDR (ENDPOINT_DIR_IN | 6)
