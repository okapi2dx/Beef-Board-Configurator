...
enum class TurntableCurve : uint8_t { Linear, Precision, Dynamic };
...
struct config {
  ...
  uint8_t tt_leds;
  HSV bar_static_hsv;
  uint8_t link_bar_effect;
  uint8_t digital_tt;
  uint8_t tt_delay_ms;
  uint8_t button_led_fade_ms;
  uint8_t button_led_brightness;
  uint8_t button_led_invert;
  uint8_t button_led_individual;
  uint8_t button_led_fade[BUTTONS];
  uint8_t button_led_level[BUTTONS];
  uint8_t button_led_inverted[BUTTONS];
  TurntableCurve tt_curve;
