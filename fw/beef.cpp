...
ButtonLedEnvelope button_led_envelopes[BUTTONS];
volatile diagnostics_report diagnostics{};
...
const FirmwareMetadata firmware_metadata PROGMEM = {
  {'B', 'E', 'E', 'F', 'V', 'E', 'R', '1'},
  FW_VER,
  FW_VERSION_MAJOR,
  FW_VERSION_MINOR,
...
// Keep the backing buffer as large as `config`, since this HID interface also
// serves the config feature report over endpoint 0. The interrupt endpoint is
// used only for the much smaller diagnostics input report.
HidReport<config, INTERFACE_ID_Config, CONFIG_IN_EPADDR> config_hid_report;
...
int main() {
  ...
  while (true) {
    ...
    // Sample physical inputs before servicing USB. This prevents a host-side
    // transfer from delaying the first observation of a short button press.
    process_buttons();
    process_combos();
    // Generate turntable/axis state before servicing the HID IN endpoint so
    // digital TT buttons are included in the report sent during this loop.
    usb_handler->update(current_config);
    usb_handler->usb_task(current_config);
    // Non-blocking interrupt-IN diagnostics for the desktop monitor.
    HID_Device_USBTask(&config_hid_report.HID_Interface);
  }
}
...
void usb_init(config &config) {
  switch (button_state) {
    case BUTTON_1 | BUTTON_8:
      set_controller_type(config, ControllerType::Default);
    ...
    case BUTTON_1 | BUTTON_9:
      set_controller_type(config, ControllerType::IIDXEntry);
      set_input_mode(config, InputMode::Joystick);
      break;
...
void EVENT_USB_Device_ConfigurationChanged() {
  ...
  Endpoint_ConfigureEndpoint(CONFIG_IN_EPADDR, EP_TYPE_INTERRUPT, HID_EPSIZE, 1);
...
bool CALLBACK_HID_Device_CreateHIDReport(USB_ClassInfo_HID_Device_t* const HIDInterfaceInfo,
  ...
  switch (ReportType) {
    case HID_REPORT_ITEM_In:
      if (HIDInterfaceInfo == &config_hid_report.HID_Interface) {
        *ReportID = HID_REPORTID_DiagnosticsStream;
        ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
          memcpy(ReportData, (const void*)&diagnostics, sizeof(diagnostics));
        }
        *ReportSize = sizeof(diagnostics);
        return false;
      }
    ...
    case HID_REPORT_ITEM_Feature:
      switch (*ReportID) {
        ...
        case HID_REPORTID_FirmwareVersion: {
          const uint32_t firmware_version = pgm_read_dword(&firmware_metadata.commit_hash);
          memcpy(ReportData, &firmware_version, sizeof(firmware_version));
          auto* firmware_info = static_cast<uint8_t*>(ReportData);
          firmware_info[4] = pgm_read_byte(&firmware_metadata.major);
          firmware_info[5] = pgm_read_byte(&firmware_metadata.minor);
          firmware_info[6] = pgm_read_byte(&firmware_metadata.patch);
          *ReportSize = sizeof(firmware_version) + 3;
          return false;
        }
        case HID_REPORTID_Diagnostics:
          ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { memcpy(ReportData, (const void*)&diagnostics, sizeof(diagnostics)); }
          *ReportSize = sizeof(diagnostics);
          return false;
        default:
...
void process_keyboard(Beef::USB_KeyboardReport_Data_t* const hid_key_codes,
  ...
  for (uint8_t i = 0; i < n; i++) {
    if (buttons & (uint16_t(1) << i)) {
      hid_key_codes->KeyCode[used_key_codes++] = key_codes[i];
    }
  }
}

uint16_t remap_buttons(const uint16_t buttons, const uint8_t* mapping) {
  constexpr uint16_t physical_mask = (uint16_t(1) << BUTTONS) - 1;
  uint16_t remapped = buttons & ~physical_mask;
  for (uint8_t physical = 0; physical < BUTTONS; ++physical) {
    if (buttons & (uint16_t(1) << physical)) {
      remapped |= uint16_t(1) << mapping[physical];
    }
  }
  return remapped;
}
...
void update_button_lighting(uint16_t led_state) {
  ...
  if (current_config.disable_leds ||
    ...
    for (auto &led : button_led_envelopes) led.reset();
    button_led_pwm_clear();
    return;
  }
  ...
  for (uint8_t i = 0; i < BUTTONS; i++) {
    bool on = (led_state >> i) & 1;
    on ^= current_config.button_led_invert;
    button_led_pwm_set(i, button_led_envelopes[i].update(on, now,
      current_config.button_led_fade_ms_extended, current_config.button_led_brightness));
  }
}
